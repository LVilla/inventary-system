...
<?php
 if(isset($_POST['add_purchase'])){
   $file = new Media();
     $file->upload($_FILES['archivo']);
     if($file->process_pdf()){
        $session->msg('s','Arhivo subido al servidor.');
        redirect('purchases.php');
     } else{
      $session->msg('d',join($file->errors));
      redirect('purchases.php');
     }
   $req_fields = array('proveedor','n_fact','purchase-description','date','price');
   ...
   if(empty($errors)){
     ...
     $query .=" proveedor,n_fact,description,date,price";
     $query .=") VALUES (";
     $query .=" '{$p_proveedor}','{$p_n_fact}', '{$p_description}', '{$p_date}', '{$p_price}'";
     $query .=")";
          ...
          <form method="post" action="add_purchase.php" class="clearfix">
              <div class="form-group">
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" name="proveedor" placeholder="Nombre de Proveedor">
               </div>
              </div>
              <div class="form-group">
                <div class="row">
                ...
                <div class="row">
                  <div class="col-md-4">
                    <div class="input-group">
                     ...
                     <input type="text" class="form-control" name="price" placeholder="Precio de compra">
                  </div>
                  </div>
                  <div class="col-md-8">
                    <div class="input-group">
                     ...
                     <input type="file" class="form-control" name="archivo" placeholder="subir archivo">
                  </div>
                  </div>
                </div>
              </div>
             
              <button type="submit" name="add_purchase" class="btn btn-danger">Agregar Compra</button>
          </form>
         </div>
        </div>
      </div>
