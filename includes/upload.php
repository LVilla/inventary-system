...
class  Media {
  ...
  public $purchasePath = SITE_ROOT.DS.'..'.DS.'uploads/facturas';
 ...
 /*--------------------------------------------------------------*/
  public function process_pdf(){
    ...
    if(!is_writable($this->purchasePath)){
        $this->errors[] = $this->purchasePath." Debe tener permisos de escritura!!!.";
        return false;
      }

    if(file_exists($this->purchasePath."/".$this->fileName)){
      $this->errors[] = "El archivo {$this->fileName} Realmente existe.";
      return false;
    }

    if(move_uploaded_file($this->fileTempPath,$this->purchasePath.'/'.$this->fileName))
    {
...
/*--------------------------------------------------------------*/
/* Function for insert pdf file
/*--------------------------------------------------------------*/
  private function insert_file(){

         global $db;
         $sql  = "INSERT INTO purchases ( file )";
         $sql .=" VALUES ";
         $sql .="(
                  '{$db->escape($this->fileName)}',
                  )";
